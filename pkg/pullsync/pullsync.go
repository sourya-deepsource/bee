...
// client
func (s *Syncer) SyncInterval(ctx context.Context, peer swarm.Address, bin uint8, fro, to uint64) (topmost uint64, err error) {
	...
	rangeMsg := &pb.GetRange{Bin: int32(bin), From: fro, To: to}
	...
	for ctr > 0 {
		...
		ch := swarm.NewChunk(addr, delivery.Data)

		if err = s.storage.Put(ctx, storage.ModePutSync, ch); err != nil {
...
// server
func (s *Syncer) makeOffer(ctx context.Context, rn pb.GetRange) (o *pb.Offer, err error) {
